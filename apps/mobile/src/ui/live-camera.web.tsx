...
const captureJpeg = (canvas: HTMLCanvasElement, timeoutMs = 2_000) =>
  new Promise<Blob>((resolve, reject) => {
    const timeout = window.setTimeout(() => {
      reject(new Error('The camera did not respond while capturing a frame. Turn it on again and retry.'));
    }, timeoutMs);
    canvas.toBlob((blob) => {
      window.clearTimeout(timeout);
      if (!blob || blob.size <= 512) {
      reject(new Error('Could not capture a valid frame. Keep this tab on the main screen and try again.'));
        return;
      }
      resolve(blob);
    }, 'image/jpeg', 0.82);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        const handler = () => {
          if (active) onError('The camera was disconnected by the browser or another app. Try again.');
        };
        ...
        const message = value instanceof DOMException && value.name === 'NotAllowedError'
          ? 'Camera permission is blocked. Allow camera access in the address bar and try again.'
          : value instanceof Error ? value.message : 'Could not open the camera in this browser.';
        onError(message);
      }
    };
    void start();
  ...
  useImperativeHandle(ref, () => ({
    recordAsync: async ({ maxDuration, maxFrames = 56 }) => {
      ...
      if (!stream || !video || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || !video.videoWidth) {
        const message = 'The camera is not ready. Wait for the preview to appear and try again.';
        onError(message);
        throw new Error(message);
      }
      if (!stream.getVideoTracks().some((track) => track.readyState === 'live')) {
        const message = 'The camera stream stopped. Turn the camera on again and retry.';
        onError(message);
        throw new Error(message);
      }
      ...
      // The VSL-30 extractor expects a stable 4:3 crop. Center-cropping here
      // avoids stretching a widescreen webcam frame before landmark extraction.
      ...
      if (!context) throw new Error('Could not initialize frame capture.');
      ...
      // Capture at a predictable cadence; the backend later selects at most
      // 32 frames for MediaPipe and resamples the sequence to 48 model steps.
      ...
      while (!stopRequestedRef.current && performance.now() < deadline && frames.length < maxFrames) {
        if (video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || !stream.getVideoTracks().some((track) => track.readyState === 'live')) {
          const message = 'The camera stream was interrupted during recording. Try again.';
          onError(message);
          throw new Error(message);
        }
      ...
      if (frames.length < 4) {
        const message = 'The camera did not capture enough frames. Hold the sign for at least one second.';
        onError(message);
        throw new Error(message);
      }
      return { frames };
